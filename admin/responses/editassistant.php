...
class EditAssistant extends \BuddyBot\Admin\Responses\MoRoot
{
    ...
    public function getDeepSeekModels() // P2d03
    {
        $this->checkNonce('get_deepseek_models');
    
        if(empty($this->api_key)){
            $this->response['success'] = false;
            $this->response['message'] = esc_html__('To create or edit an assistant, you need to configure your DeepSeek API key in the BuddyBot settings.', 'buddybot-ai-custom-ai-assistant-and-chat-agent');
            $this->response['empty_key'] = true;
            $this->response['html'] = '<option value="" disabled selected>' . esc_html__('No API key configured', 'buddybot-ai-custom-ai-assistant-and-chat-agent') . '</option>';
            echo wp_json_encode($this->response);
            wp_die();
        ...
        if ($this->deepseek_response_body->object === 'list') {
            $this->response['success'] = true;
            $this->response['list'] = $this->deepseek_response_body->data;
            $this->response['html'] = $this->modelsListHtml($this->deepseek_response_body->data);
        } else {
            $this->response['success'] = false;
            $this->response['message'] = esc_html__('Unable to fetch models list.', 'buddybot-ai-custom-ai-assistant-and-chat-agent');
        }
    ...
    public function createDeepSeekAssistant() // Pfdc9
    {
        $this->checkNonce('create_deepseek_assistant');
        $this->checkCapabilities();
        ...
        $url = 'https://api.deepseek.com/v1/assistants' . $assistant_id;
        ...
        $this->deepseek_response = wp_remote_post($url, $args);
        $this->processResponse();
    ...
    public function __construct()
    {
        ...
        add_action('wp_ajax_getDeepSeekModels', array($this, 'getDeepSeekModels')); // P2d03
        add_action('wp_ajax_createDeepSeekAssistant', array($this, 'createDeepSeekAssistant')); // Pfdc9
    }
}
